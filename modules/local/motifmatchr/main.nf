process MOTIFMATCHR_MATCHMOTIFS {
    ...
    regions <- GRanges(seqnames = peaks[[1L]], ranges = IRanges(start = as.integer(peaks[[2L]]) + 1L, end = as.integer(peaks[[3L]])), region_id = region_id)
    ...
    write.table(data.frame(row_index = seq_len(length(regions)), region_id, chrom = as.character(seqnames(regions)), start = start(regions) - 1L, end = end(regions)), paste0(out_prefix, '.motifmatchr_peaks.tsv'), quote = FALSE, row.names = FALSE, sep = '\t')
    write.table(data.frame(column_index = seq_len(ncol(match_matrix)), motif_id = colnames(match_matrix)), paste0(out_prefix, '.motifmatchr_motifs.tsv'), quote = FALSE, row.names = FALSE, sep = '\t')
    RSCRIPT
