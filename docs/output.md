...
- `<ALIGNER>/merged_library/tobias/atacorrect/`
  - `*_corrected.bw`, `*_expected.bw`: TOBIAS ATACorrect signal tracks for each merged library-level sample.
  - `*_uncorrected.bw`, `*_bias.bw`: Optional ATACorrect diagnostic tracks.
- `<ALIGNER>/merged_library/tobias/scorebigwig/`
  - `*_footprints.bw`: TOBIAS ScoreBigwig footprint scores for each merged library-level sample.
- `<ALIGNER>/merged_library/tobias/bindetect/`
  - `bindetect/`: TOBIAS BINDetect output directory generated across all merged library-level samples.
  - `bindetect_results.txt`, `bindetect_results.xlsx`: Transcription factor binding results.
  - `bindetect_figures.pdf`: Static BINDetect summary figures.
  - `bindetect_distances.txt`: Motif clustering distance matrix.
  - `bindetect_*.html`: Interactive BINDetect plots, when generated by TOBIAS.
...
## Optional Genetic Analyses

The genetic layer uses a BWA/GATK-preprocessed BAM stream that is separate from the main chromatin-accessibility alignment path. This keeps accessibility analyses and variant/CNV/telomere-oriented analyses explicit while allowing both layers to share the same input libraries and selected consensus peak regions where biologically useful.
...
- `variants/alignment/bwa/<SAMPLE>/`
  - `*.bam`, `*.bai`: BWA-MEM alignment files used by the genetic layer.
- `variants/markduplicates/<SAMPLE>/`
  - `*.metrics`: GATK/Picard duplicate marking metrics.
- `variants/bam/<SAMPLE>/`
  - `*.md.bam`, `*.md.bam.bai`: Analysis-ready BAM and index after duplicate marking and optional BQSR.
- `variants/vcf/<CALLER>/<SAMPLE>/`
  - `*.vcf.gz`, `*.vcf.gz.tbi`: Raw VCF files generated by the selected caller.
- `variants/regions/`
  - `consensus_peaks.variant_filter.slop<N>.bed`: Consensus peak regions used when filtering variants to accessible chromatin.
- `variants/vcf_in_peaks/<CALLER>/<SAMPLE>/`
  - `*.peaks.vcf.gz`, `*.peaks.vcf.gz.tbi`: VCF files restricted to widened consensus peak regions when `--variants_in_peaks_only` is enabled.
- `variants/stats/<CALLER>/<SAMPLE>/`
  - `*.bcftools_stats.txt`: `bcftools stats` output for final VCF files.
  - `*.variant_counts_mqc.tsv`: MultiQC-compatible variant count summary.
- `variants/vep/<SAMPLE>/`
  - VEP-annotated VCF and HTML report when annotation is enabled.
- `variants/maf/<SAMPLE>/`
  - MAF files generated from annotated VCF files when annotation is enabled.
- `variants/oncoplot/`
  - maftools oncoplot outputs when `--run_oncoplot` is enabled.

</details>

If `--run_variants` is specified, the pipeline runs BWA-MEM and GATK preprocessing on the input libraries before short-variant calling. The callers are selected with `--variant_callers`; available values are `haplotypecaller`, `freebayes`, `bcftools` and `deepvariant`. `--variants_in_peaks_only` can be used to keep only variants located in merged-library consensus peaks widened by `--peak_filter_slop`. Variant annotation with VEP/vcf2maf is optional and can be skipped with `--skip_variant_annotation`.
